...
class TestToastManager @Inject constructor() : ToastManager {
  ...
  fun assertError(expectedMessage: String? = null) {
    if (expectedMessage == null) {
      ...
      check(hasError) { "Expected an error message but got: ${messages.map { it.first }}" }
    } else {
      val error =
          messages.firstOrNull { (msg, type) ->
  ...
  fun assertInfo(expectedMessage: String) {
    val info =
        messages.firstOrNull { (msg, type) -> msg == expectedMessage && type == MessageType.INFO }
    check(info != null) {
