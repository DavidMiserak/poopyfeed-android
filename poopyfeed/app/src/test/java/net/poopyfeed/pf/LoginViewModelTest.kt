...
class LoginViewModelTest {
    ...
    fun `login success saves token and emits Success`() = runTest {
        ...
        coEvery { anyConstructed<AuthRepository>().login(any(), any()) } returns ApiResult.Success("token-123")
    ...
    fun `login error emits Error state with message`() = runTest {
        ...
        coEvery { anyConstructed<AuthRepository>().login(any(), any()) } returns ApiResult.Error(apiError)
    ...
    fun `clearError from Error state sets Idle`() = runTest {
        every { NetworkModule.getAuthToken(any()) } returns null
        coEvery { anyConstructed<AuthRepository>().login(any(), any()) } returns ApiResult.Error(
