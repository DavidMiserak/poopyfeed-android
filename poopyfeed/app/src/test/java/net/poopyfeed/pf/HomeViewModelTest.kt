...
class HomeViewModelTest {
    ...
    @Test
    fun `when token is missing then Unauthorized state emitted without calling repository`() = runTest {
        ...
        coEvery { anyConstructed<AuthRepository>().getProfile() } returns ApiResult.Success(
            UserProfile(
                id = 1,
    ...
    @Test
    fun `when profile request returns 401 then token cleared and Unauthorized state emitted`() = runTest {
        every { NetworkModule.getAuthToken(any()) } returns "test-token"

        val httpError = ApiError.HttpError(
            statusCode = 401,
