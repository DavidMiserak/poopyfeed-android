...
class NotificationsPagingSourceTest {
  ...
  fun `load network error returns Error result`() = runTest {
    coEvery { mockApiService.listNotifications(1, 20) } throws java.io.IOException("Network error")
  ...
  fun `load http error returns Error result`() = runTest {
    val errorResponse = retrofit2.Response.error<Any>(401, "Unauthorized".toResponseBody(null))
  ...
  fun `getRefreshKey returns correct page key`() {
    ...
    val page =
        PagingSource.LoadResult.Page(
            data = listOf(mockNotification1, mockNotification2), prevKey = 1, nextKey = 3)
