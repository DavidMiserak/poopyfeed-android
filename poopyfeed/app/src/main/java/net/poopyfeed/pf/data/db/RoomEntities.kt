...
/**
 * Room database entities that mirror API models.
 * Stored in local SQLite for offline access and caching.
 */

/**
 * Child entity for Room database.
 * Maps 1:1 to API Child model.
 */
...
data class ChildEntity(
    ...
    companion object {
        fun fromApiModel(child: Child): ChildEntity = ChildEntity(
            id = child.id,
            name = child.name,
...
data class FeedingEntity(
    ...
    fun toApiModel(): Feeding = Feeding(
        id = id,
        child = child,
    ...
    companion object {
        fun fromApiModel(feeding: Feeding): FeedingEntity = FeedingEntity(
            id = feeding.id,
            child = feeding.child,
...
data class DiaperEntity(
    ...
    companion object {
        fun fromApiModel(diaper: Diaper): DiaperEntity = DiaperEntity(
            id = diaper.id,
            child = diaper.child,
...
data class NapEntity(
    ...
    companion object {
        fun fromApiModel(nap: Nap): NapEntity = NapEntity(
            id = nap.id,
            child = nap.child,
