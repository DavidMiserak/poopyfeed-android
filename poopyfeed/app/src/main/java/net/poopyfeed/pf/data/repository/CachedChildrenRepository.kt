...
class CachedChildrenRepository(
     ...
     * ```
     */
    fun listChildrenCached(): Flow<ApiResult<List<Child>>> = childDao.getAllChildrenFlow()
        .map { entities ->
            if (entities.isEmpty()) {
                ApiResult.Loading()
            } else {
    ...
    /**
     * Manually refresh children from API and update cache.
     * Useful for pull-to-refresh patterns.
     */
    suspend fun refreshChildren(): ApiResult<List<Child>> = try {
    ...
    /**
     * Get single child from cache as Flow.
     * Emits updates whenever the child data changes in Room.
     */
    fun getChildCached(childId: Int): Flow<Child?> = childDao.getChildFlow(childId)
        .map { it?.toApiModel() }

    /**
     * Create child: API-first, then cache.
     */
    suspend fun createChild(request: CreateChildRequest): ApiResult<Child> = try {
     ...
     */
    suspend fun deleteChild(childId: Int): ApiResult<Unit> = try {
...
class CachedFeedingsRepository(
    ...
    fun listFeedingsCached(childId: Int): Flow<ApiResult<List<Feeding>>> =
        feedingDao.getFeedingsFlow(childId)
            .map { entities ->
                if (entities.isEmpty()) {
                    ApiResult.Loading()
                } else {
     ...
     */
    suspend fun refreshFeedings(childId: Int): ApiResult<List<Feeding>> = try {
     ...
     */
    suspend fun deleteFeeding(childId: Int, feedingId: Int): ApiResult<Unit> = try {
...
class CachedDiapersRepository(
    ...
    fun listDiapersCached(childId: Int): Flow<ApiResult<List<Diaper>>> =
        diaperDao.getDiapersFlow(childId)
            .map { entities ->
                if (entities.isEmpty()) {
                    ApiResult.Loading()
                } else {
    ...
    suspend fun refreshDiapers(childId: Int): ApiResult<List<Diaper>> = try {
    ...
    suspend fun deleteDiaper(childId: Int, diaperId: Int): ApiResult<Unit> = try {
...
class CachedNapsRepository(
    ...
    suspend fun refreshNaps(childId: Int): ApiResult<List<Nap>> = try {
    ...
    suspend fun deleteNap(childId: Int, napId: Int): ApiResult<Unit> = try {
