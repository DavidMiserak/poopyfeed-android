 ...
 * Each DAO provides suspend functions and Flow queries for local data access.
 */

/**
 * DAO for Child entities.
 * Provides CRUD operations and queries for children.
 */
@Dao
interface ChildDao {

    /**
     * Get all children as a Flow (updates automatically when data changes).
     * Useful for UI that should update in real-time.
     */
    @Query("SELECT * FROM children ORDER BY name ASC")
    fun getAllChildrenFlow(): Flow<List<ChildEntity>>

    /**
     * Get all children as a one-time suspend query.
     * Useful when you only need data once.
     */
    @Query("SELECT * FROM children ORDER BY name ASC")
...
interface FeedingDao {
     ...
     */
    @Query(
        "SELECT * FROM feedings WHERE child = :childId ORDER BY timestamp DESC"
    )
     ...
     */
    @Query(
        "SELECT * FROM feedings WHERE child = :childId ORDER BY timestamp DESC LIMIT 1"
    )
     ...
     */
    @Upsert
    suspend fun upsertFeeding(feeding: FeedingEntity)

    /**
     * Batch upsert feedings.
     */
    @Upsert
    suspend fun upsertFeedings(feedings: List<FeedingEntity>)

    /**
     * Delete a feeding by ID.
     */
    @Query("DELETE FROM feedings WHERE id = :feedingId")
    suspend fun deleteFeeding(feedingId: Int)

    /**
     * Clear all feedings for a child.
     */
    @Query("DELETE FROM feedings WHERE child = :childId")
    suspend fun clearChildFeedings(childId: Int)
}
...
interface DiaperDao {
     ...
     */
    @Query(
        "SELECT * FROM diapers WHERE child = :childId ORDER BY timestamp DESC"
    )
     ...
     */
    @Query(
        "SELECT * FROM diapers WHERE child = :childId ORDER BY timestamp DESC LIMIT 1"
    )
     ...
     */
    @Upsert
    suspend fun upsertDiaper(diaper: DiaperEntity)

    /**
     * Batch upsert diapers.
     */
    @Upsert
    suspend fun upsertDiapers(diapers: List<DiaperEntity>)

    /**
     * Delete a diaper by ID.
     */
    @Query("DELETE FROM diapers WHERE id = :diaperId")
    suspend fun deleteDiaper(diaperId: Int)

    /**
     * Clear all diapers for a child.
     */
    @Query("DELETE FROM diapers WHERE child = :childId")
    suspend fun clearChildDiapers(childId: Int)
}
...
interface NapDao {
     ...
     */
    @Query(
        "SELECT * FROM naps WHERE child = :childId ORDER BY start_time DESC"
    )
     ...
     */
    @Query(
        "SELECT * FROM naps WHERE child = :childId ORDER BY start_time DESC LIMIT 1"
    )
     ...
     */
    @Query(
        "SELECT * FROM naps WHERE child = :childId AND end_time IS NULL"
    )
     ...
     */
    @Upsert
    suspend fun upsertNap(nap: NapEntity)

    /**
     * Batch upsert naps.
     */
    @Upsert
    suspend fun upsertNaps(naps: List<NapEntity>)

    /**
     * Delete a nap by ID.
     */
    @Query("DELETE FROM naps WHERE id = :napId")
    suspend fun deleteNap(napId: Int)

    /**
     * Clear all naps for a child.
     */
    @Query("DELETE FROM naps WHERE child = :childId")
    suspend fun clearChildNaps(childId: Int)
}
